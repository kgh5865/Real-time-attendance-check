...
type MainScreenRouteProp = RouteProp<RootStackParam, 'AdminInfor'>;
    ...
    try {
      ...
      if (response.data && response.data.body) {
        ...
        if (userArray && userArray.length > 0) {
          ...
          setAdminData(firstUser); // 수정된 부분: setAdminData로 변경
        } else {
    ...
    <View style={styles.container}>
      {adminData && (
        <View style={styles.userDataContainer}>
          ...
          <Text style={styles.userData}>{adminData.admin_id}</Text>

          <Text style={styles.userDataLabel}>이름:</Text>
          <Text style={styles.userData}>{adminData.name}</Text>

          <Text style={styles.userDataLabel}>학과:</Text>
          <Text style={styles.userData}>{adminData.department}</Text>
        </View>
      )}
    </View>
  );
